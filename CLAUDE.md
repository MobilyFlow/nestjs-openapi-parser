...
- **Component keys are sanitized.** OpenAPI component keys must match `^[a-zA-Z0-9._-]+$`, but TS identifiers allow more (notably `$`, as in `AppEvent$WebhookPayload`). `toComponentName` in [src/parser/schema-builder.ts](src/parser/schema-builder.ts) maps the TS name to a valid key, applied at exactly the emission points — the `components.schemas` key and every `$ref` — while AST lookups (`hasModel`/`getModel`, the `pending`/`done` queues) stay keyed by the original TS name. Two distinct models sanitizing to the same key **throw** (collision check in `build()`).
